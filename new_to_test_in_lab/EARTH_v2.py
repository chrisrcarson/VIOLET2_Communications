...
from earth_utils_v2 import *
from earth_utils_v2 import _buildViolet2Header, _padApplicationData
...
try:
    while not isExiting:
        ...
        if userInput.lower() == "ping": # local command: send a ping to VIOLET2 and measure round-trip time
            ...
            pingRetryCount = 0
            pingSuccess = False
            while not pingSuccess and pingRetryCount <= COMMAND_MAX_RETRIES:
                ...
                try:
                    ...
                    if "error" not in parsed and parsed["msg_type"] == MSG_PONG:
                        ...
                        pingSuccess = True  # got something back, don't retry
                except socket.timeout:
                    pingRetryCount += 1
                    if pingRetryCount <= COMMAND_MAX_RETRIES:
                        print(f"Ping timed out (attempt {pingRetryCount}/{COMMAND_MAX_RETRIES}), retransmitting...")
                    else:
                        print(f"Ping timed out: no response from VIOLET2 after {COMMAND_MAX_RETRIES + 1} attempts")
            continue
        ...
        try:
            while not responseComplete and retryCount <= COMMAND_MAX_RETRIES:
                try:
                    ...
                    retryCount += 1
                    if retryCount <= COMMAND_MAX_RETRIES:
                        print(f"Timeout (attempt {retryCount}/{COMMAND_MAX_RETRIES}), retransmitting...")
                        flushSocket()
                        sendCommand()
                    else:
                        print(f"Connection Timeout: No response after {COMMAND_MAX_RETRIES} attempts")
                        flushSocket()
